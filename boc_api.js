...
const alpaca = new Alpaca({
    keyId: 'PKSUN7CKUEBS47OSEWAN',
    secretKey: '/R8INcteekjVjtokraKNUd1zcCmdCYKivTkgItuo',
    paper: true,
});
...
alpaca.getAccount().then((account) => {
    ...
    boc_api.alpaca_account = account;
});

boc_api.get_apple = function() {
    return new Promise((resolve, reject) => {
        const barset = alpaca.getBars(
            ...
            resolve(`AAPL moved ${parseFloat(percent_change).toFixed(4)}% over the last 5 days`);
        });
    });
}
...
boc_api.getAccountStatements = function(accountId,startDate="01/01/2019",endDate="31/12/2020",maxCount=0){
